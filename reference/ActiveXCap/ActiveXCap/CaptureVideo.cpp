...
public:
	long lWidth;
	long lHeight;
	wchar_t m_szFileName[MAX_PATH];// 位图文件名称
	CSampleGrabberCB( ){
		wcscpy(m_szFileName, L"c:\\captmp.bmp");
	}
	...
	{
		wcscpy(m_szFileName, strFileName);
	}
...
HRESULT CCaptureVideo::Init(int iDeviceID, HWND hWnd)
{
	...
	if( FAILED( hr ) ){
		AfxMessageBox(L"Fail to set media type!");
		return hr;
	}
	...
	if ( FAILED( hr) ){
		AfxMessageBox(L"Failt to read the connected media type");
		return hr;
	}
...
//设置文件的保存文件
void CCaptureVideo::SetSaveFilename(CString strFileName)
{
	mCB.SetSaveFileName(strFileName);
}
